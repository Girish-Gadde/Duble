...
const HARD_CODED_NAME = "9112446365";

export default function JoinTeam({ mobileNumber, navigation }) {
    ...
    try {
      ...
      if (response.data.message) {
        ...
        if (response.data.error === "Team already has two members") {
          Alert.alert(
            "Team Full",
            "This team is already full. Please try joining another team."
          );
        } else {
      ...
      Alert.alert(
        "Error",
        "Failed to join team. Please check your details and try again."
      );
    }
  };
