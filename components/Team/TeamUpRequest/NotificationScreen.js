...
const AcceptInviteScreen = () => {
  ...
  const invitee = "9112446365"; // Assuming the logged-in user is Rohit
    ...
    const fetchInvites = async () => {
      try {
        const response = await axios.get(
          `${serverIP}/notification/get-invites/${invitee}`
        );
        setInvites(response.data.receivedRequests);
        console.log(response.data, "RESq");
      } catch (error) {
        console.error(
  ...
  const acceptInvite = async (teamId) => {
    try {
      const response = await axios.post(
        `${serverIP}/notification/accept-invite`,
        {
          teamId,
          invitee,
        }
      ...
      Alert.alert(
        "Success",
        `Invite accepted! Team members: ${response.data.team.members.join(
          ", "
        )}`
      );
          ...
          <View key={index} style={styles.inviteCard}>
            ...
            <View style={styles.buttonContainer}>
              ...
              <Button
                title="Reject"
                color="red"
                onPress={() => rejectInvite(invite._id)}
              />
            </View>
          </View>
        ))
