...
const AcceptInviteScreen = () => {
  ...
  useEffect(() => {
    const fetchInvites = async () => {
      try {
        const response = await axios.get(`http://192.168.1.19:3000/get-invites/${invitee}`);
  ...
  const acceptInvite = async (teamId) => {
    try {
      const response = await axios.post('http://192.168.1.19:3000/accept-invite', {
        teamId,
        invitee
      });
  ...
  const rejectInvite = async (teamId) => {
    try {
      await axios.post('http://192.168.1.19:3000/reject-invite', {
        teamId,
