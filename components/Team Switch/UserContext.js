...
export const UserProvider = ({ children }) => {
    ...
    const saveSelectedIndex = async () => {
      try {
        if (selectedTeamIndex !== null) {
          await AsyncStorage.setItem('selectedTeamIndex', selectedTeamIndex.toString());
          console.log("Saved Selected Team Index to AsyncStorage:", selectedTeamIndex);
        }
      } catch (error) {
  ...
  return (
    <UserContext.Provider value={{ selectedTeamIndex, setSelectedTeamIndex, indexRef }}>
      {children}
